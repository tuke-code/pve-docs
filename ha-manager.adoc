...
include::generated/ha-manager.1-synopsis.adoc[]
...
xref:ha_manager_node_affinity_rules[node affinity rule]. The HA Manager then
...
include::generated/ha-resources-opts.adoc[]
...
include::generated/ha-groups-opts.adoc[]
...
include::generated/ha-rules-opts.adoc[]
...
include::generated/ha-rules-node-affinity-opts.adoc[]
...
include::generated/ha-rules-resource-affinity-opts.adoc[]
...
See xref:ha_manager_disarm[Disarming HA for Cluster Maintenance].
...
xref:ha_manager_crs_scheduling_points[CRS Scheduling Points] for a more thorough
...
[[ha_manager_crs_basic_scheduler]]
Basic Scheduler
^^^^^^^^^^^^^^^
...
[[ha_manager_crs_static_scheduler]]
...
[[ha_manager_crs_dynamic_scheduler]]
...
For the xref:ha_manager_crs_basic_scheduler[basic scheduler mode], the node with
the fewest active guests is considered the best-suited node.

For the xref:ha_manager_crs_static_scheduler[static-load] and
xref:ha_manager_crs_dynamic_scheduler[dynamic-load] scheduler modes, each node in turn is
...
IMPORTANT: For the static-load and dynamic-load scheduler modes, this
...
NOTE: Any HA resource with a positive affinity for an HA resource with
...
requires either the xref:ha_manager_crs_static_scheduler[static-load] or
xref:ha_manager_crs_dynamic_scheduler[dynamic-load] scheduler mode to be in use.
...
_imbalance threshold_. If the cluster imbalance stays above the threshold for
at least as many consecutive HA Manager rounds as the _hold duration_, then
...
the best rebalancing migration can be set with the _rebalancing method_
option. Finally, if the selected migration reduces the current cluster
imbalance by at least the percentage given by the _imbalance improvement_
...
individual node loads and is normalized to a 0-100% range. Both the _imbalance
threshold_ and the _imbalance improvement_ are configured in percent. The
_imbalance threshold_ defaults to 30% and controls the sensitivity of the load
balancer; with a value of `0` it will try to select a rebalancing migration
every time the _hold duration_ is exceeded. The _imbalance improvement_
