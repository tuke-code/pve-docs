...
include::generated/pvecm.1-synopsis.adoc[]
...
.. Start by xref:pve_ceph_osd_destroy[destroying] the remaining Ceph OSDs on the
   node to be deleted, one after another.

.. Wait until the xref:pve_ceph_mon_and_ts[CEPH status] reaches `HEALTH_OK`
   again.

.. If any exists, destroy the remaining xref:pveceph_fs_mds[Ceph metadata servers]
...
.. xref:pveceph_destroy_mon[Destroy the remaining Ceph monitor.]

.. xref:pveceph_destroy_mgr[Destroy the remaining Ceph manager.]

.. Finally, remove the now empty Ceph bucket ({pve} node to be removed) from the
   CRUSH hierarchy.
+
----
...
* The SSH fingerprint of the removed node will still reside in the
  ...
  cluster wide.
* Existing xref:ha_manager_rules[high availability rules] will still
...
footnote:[`token_coefficient` in the corosync manual page
