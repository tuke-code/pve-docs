...
Add an xref:ceph_rados_block_devices[RBD storage] in the web interface to use
a Ceph pool for virtual machine and container disks. For an external cluster,
follow the xref:storage_rbd_auth[authentication instructions] to configure a
dedicated Ceph user and its keyring.
...
Muting a health check keeps it visible but excludes it from the overall health
status, making new problems easier to notice. Ceph continues evaluating the
check. Use a duration so that a deferred warning returns automatically, for
example after two weeks:
...
In the web interface, expand the check under **Datacenter -> Ceph**, click
'Mute', and set a duration. The check stays in the list, marked as muted; use
'Unmute' in the same place to restore it earlier.

By default, a mute clears when more items become affected. Enable 'Keep Mute',
or add `--sticky` to the command, only if the check should stay muted in that
case. This does not change the duration: a timed sticky mute still expires.

NOTE: Muting does not resolve the condition. You accept its risk for as long
as the mute lasts.
...
Two checks set the cluster status to 'HEALTH_ERR'. This does not mean that
storage access or a Ceph service has failed. Follow the steps below to migrate
to `aes256k` and, when all clients support it, disable the vulnerable method.
...
node, keyring, and device inventories and inspection diagnostics.
...
.Before You Begin
* Upgrade `pve-manager` to 9.2.19 or newer and install the latest Ceph
...
* If Cephx is disabled on this cluster, follow
  xref:pveceph_cephx_disabled[Clusters with Cephx Disabled] instead.
...
If the plan reports no blocker, apply the same selection:
...
warning about rotating service keys can remain for a few hours while old
rotating keys expire. You do not need to wait for this warning to clear before
...
If the plan reports no blocker, apply it:
...
While a key is staged, do not add or downgrade monitors or change that user's
...
* Let backups, restores, disk imports, and clones on Ceph storage finish
  before confirmation; these operations can retain the key they started with.
...
The helper reports sessions that may still hold an old key. Client hints
are host-wide, not session attribution or a complete workload inventory.
Sockets without an owning process are listed separately. Check disconnected
clients and external key copies yourself.
...
WARNING: Clients that still need an old key or cipher cannot authenticate
...
[[pveceph_cephx_health_check_reference]]
...
|`AUTH_INSECURE_SERVICE_KEY_TYPE` |A Ceph service key uses the old cipher.
|Run the cluster-owned migration. This check has error severity.
|`AUTH_INSECURE_SERVICE_TICKETS` |Monitors issue service tickets with the old
cipher. |Run the cluster-owned migration. This check has error severity.
|`AUTH_INSECURE_CLIENT_KEY_TYPE` |The active key of a Ceph user uses the old
cipher. A staged user stays listed until its rotation is confirmed.
|Rotate the keys of compatible users and confirm them, or mute the warning.
|`AUTH_INSECURE_ROTATING_SERVICE_KEY_TYPE` |A rotating service key uses the old
cipher. |Continue the migration; old rotating keys expire automatically and do
not prevent final cipher restriction.
|`AUTH_INSECURE_KEYS_ALLOWED` |Monitors accept the old cipher. |Finish the
migration, or mute the warning while old clients remain.
|`AUTH_INSECURE_KEYS_CREATABLE` |Monitors can create old-cipher keys. |Finish
the migration, or mute the warning while old clients remain.
|===

[[pveceph_cephx_disabled]]
Clusters with Cephx Disabled
++++++++++++++++++++++++++++

Ceph evaluates these health checks from the stored keys and cipher settings,
also when Cephx is disabled, that is, when `auth_cluster_required`,
`auth_service_required`, and `auth_client_required` are set to `none`.
Disabling Cephx therefore does not clear them, and changing the authentication
method interrupts existing clients, which need a restart or remount afterwards.

On a cluster with Cephx disabled:

. Run the helper without options and apply its plan. It migrates the manager,
  metadata server, and OSD keys, restarting services one at a time when needed,
  and switches the service tickets to the new cipher. This clears both
  error-severity checks. If an old `mon.` key is stored in the authentication
  database, the helper instead refuses and asks you to migrate that key first.
. Optionally, migrate the bootstrap and crash keys with `--rotate-client-keys`.
. Mute the remaining warnings without a duration. While Cephx is disabled, the
  helper does not migrate the keys of other Ceph users, as their clients do not
  authenticate with Cephx:
+
[source,bash]
----
ceph health mute AUTH_INSECURE_CLIENT_KEY_TYPE --sticky
ceph health mute AUTH_INSECURE_KEYS_ALLOWED --sticky
ceph health mute AUTH_INSECURE_KEYS_CREATABLE --sticky
----

Do not restrict the allowed ciphers or forbid creating old-cipher keys by hand
while the preferred cipher is still `aes`, as creating keys then fails.

Before you enable Cephx again, unmute these checks and make sure that every
monitor, Ceph service, and client still has a key file matching the
authentication database. Restart the monitors first, then all other Ceph
services, and restart or remount the clients. Then run the migration as described in
xref:pveceph_cephx_migration_helper[Migration Steps].

[[pveceph_cephx_staged_keys]]
Staged Keys and Confirmation
++++++++++++++++++++++++++++

The helper disables automatic promotion of pending client keys so that the
monitors keep both credentials valid. An older monitor would promote a staged
key on its first use and end the grace period for the whole cluster. The
helper therefore stages only when every monitor supports the feature and
checks this again on every run.
...
key, allowing refreshed clients to stay connected during forward or rollback
...
* CephFS bare secret: `/etc/pve/priv/ceph/<STORAGE_ID>.secret`
...
A storage with `monhost` set counts as external even if its monitors belong to
the local cluster, so the helper does not update its key copy. If that copy
holds the key of a Ceph user you migrate, update it with the staged key of the
same user before confirming, or with the restored key before confirming a
rollback. The helper refuses to retire a key it finds in a storage key copy;
verify copies it cannot check yourself.

The helper lists local users not managed by {pve} separately. Follow their
client's supported rotation procedure, or leave their keys unchanged and keep
the old cipher enabled.

[[pveceph_cephx_rollback]]
Roll Back a Staged Client Key
+++++++++++++++++++++++++++++

To return a staged user to its current key:
...
. Run the helper with `--confirm-abort-clients-refreshed <USER> --apply` to
  retire the staged key. The helper refuses if monitor session collection is
  incomplete, a visible session uses the staged key, its key is unknown, or a
  storage key copy still holds the staged key.

Emergency Cipher Override
+++++++++++++++++++++++++

Monitors normally use the `auth_allowed_ciphers` list stored in the MonMap to
decide which key ciphers may authenticate. The local startup option
`mon_auth_emergency_allowed_ciphers` replaces that list for a monitor. It can
temporarily allow the cipher of an existing administrative key after an
incorrect restriction locked administrators out footnote:[Ceph: Emergency
Allowed Ciphers
{cephdocs-url}/rados/configuration/auth-config-ref/#emergency-allowed-ciphers].

Ceph raises `AUTH_EMERGENCY_CIPHERS_SET` while this override is configured. The
helper refuses the final restriction because changing the MonMap would not
take effect. Use the override only to recover access, then repair the normal
configuration, remove it from `/etc/pve/ceph.conf`, and restart every monitor
before the final dry run.

Interrupted Runs and Manual Fallback
++++++++++++++++++++++++++++++++++++

After an interruption, run the helper without options once the affected nodes
are reachable. Unfinished key updates can resume while their daemons are
stopped. Keep the journal unchanged and follow the printed next steps rather
than starting a manual rotation. Check the affected nodes and service logs
rather than assuming a down daemon's key needs repair.

If the helper still cannot proceed, install the latest `pve-manager` and retry.
Contact Proxmox support before editing the journal or keyrings, or before using
the upstream Cephx key migration procedure footnote:[Ceph documentation:
{cephdocs-url}/rados/configuration/auth-config-ref/#cephx-upgrade]. That
procedure changes the Ceph authentication database, but does not account for
these Proxmox VE details:

* Ensure every managed key copy below `/etc/pve/priv` and `/var/lib/ceph`
  contains the key that Ceph accepts. Merge changes into
...
Use manual key-rotation or cipher-restriction commands only with a complete,
tested recovery plan that covers every item above.
